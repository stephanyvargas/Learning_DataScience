...
prefix_list = ['I']#[chr(i) for i in range(73,91)]#from letter A -> range(65,91)
code_list = [str(s).zfill(4) for s in range(772,10000)]
    ...
    #for code in tqdm(data_into_list[:-1]):
        ...
        if all_info:
            ...
            try:
                ...
                if file_exists:
                    #open existing data
                    df_initial = pd.read_json(path, orient ='records', compression = 'infer', nrows=2, lines=True)
                    df = pd.DataFrame(all_info.values(),index=all_info.keys())
                    df_extended = df_initial.append(df)
                    df_extended.to_json(path, orient='records', compression = 'infer', index = 'true')
                else:
                    df = pd.DataFrame(all_info.values(),index=all_info.keys())
                    df.to_json(path, orient ='records', compression = 'infer', index = 'true')
                del df_initial, df, df_extended
            except:
